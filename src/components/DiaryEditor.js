...
const DiaryEditor = ( {isEdit, targetData} ) => {
    ...
    useEffect(() => {
        if(isEdit){
            setDate(getStringDate(new Date(parseInt(targetData.date))));
    ...
    const handleSubmit = () => {
        if(content.length < 1){
            contFoucs.current.focus();
            return;
        }
        if(window.confirm(isEdit ? "작성하신 일기를 수정하시겠습니까?" : "새로운 일기를 작성하시겠습니까?")){
            if(isEdit){
                onEdit(targetData.id, date, content, emotion);
            }else{
    ...
    const handleRemove = () => {
        if(window.confirm("작성하신 일기를 삭제하시겠습니까?")){
            ...
            <div>
                ...
                <section>
                    <div className="control_box">
                        <MyButton text={"취소하기"} onClick={() => navigate("/")}></MyButton>
